...
static long long int mmap_total_ticks = 0;
...
int mmap (int fd, void *vaddr)
{
  ...
  if (i < num_pages)
    ...
    int64_t end_ticks = timer_ticks();
    mmap_total_ticks += (end_ticks - start_ticks);
    //printf("Processor time for mmap setup: %lld ticks\n", mmap_total_ticks);
  return md;
}
void munmap (int md)
{
  ...
  if (md >= 0 && md < MAX_MMAP_FILES)
    ...
    int64_t end_ticks = timer_ticks();
    mmap_total_ticks += (end_ticks - start_ticks);
    //printf("Processor time for munmap: %lld ticks\n", mmap_total_ticks);
}
